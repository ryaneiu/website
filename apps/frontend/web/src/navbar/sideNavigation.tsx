...
export function SideNavigation() {
    ...
    const selectedTab = displayedTabStore(state => state.displayedTab);

    return (
        <div className="flex flex-col border-r border-r-black/15 w-fit h-full">
            {TABS.map(tab => {
                return <SideNavigationButton key={tab.id} icon={tab.icon} text={tab.text} selected={tab.id === selectedTab} onClick={() => {
                    displayedTabStore.setState({displayedTab: tab.id});
